...
def store_university_admin(user_id, university_acro):
    """
    Assigns the current user as an admin for the university.

    :param user_id: The ID of the user.
    :param university_acro: The acronym of the university.
    """
    # Retrieve the university by its acronym
    university = University.get_university_by_acronym(university_acro)

    if university:
        # Insert the new admin into the university admins table
        query(
            """
            INSERT INTO university_moderator (user, university, tier)
            VALUES (?, ?, ?);
            """,
            (user_id, university.id, 1)  # tier 1 means admin level
        )

def is_course_admin(user_id, course):
    """
    Check if the user is the admin of the given course.
    """
    course_admins = query(
        """
        SELECT user FROM course_moderators WHERE course = ?
        """,
        (course.id,)
    )

    return user_id in [admin[0] for admin in course_admins]


def is_university_admin(user_id, university):
    """
    Check if the user is the admin of the given university.
    """
    university_admins = query(
        """
        SELECT user FROM university_moderator WHERE university = ?
        """,
        (university.id,)
    )

    return user_id in [admin[0] for admin in university_admins]
