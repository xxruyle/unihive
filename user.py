...
class User:
    ...
    @property
    def created_universities(self):
        """
        Getter for universities that the user has created (and is an admin of).
        """
        return [University(*params) for params in query(
            """
            SELECT universities.id, name, acronym, description
            FROM universities
            INNER JOIN university_moderator
            ON universities.id = university_moderator.university
            WHERE university_moderator.user = ?;
            """,
            (self.id,)
        )]

    @property
    def managed_courses(self):
        """
        Getter for courses the user is an admin of (created by the user).
        """
        return [Course(*params) for params in query(
            """
            SELECT courses.id, name, description, course_number, department, university
            FROM courses
            INNER JOIN course_moderators
            ON courses.id = course_moderators.course
            WHERE course_moderators.user = ?;
            """,
            (self.id,)
        )]
