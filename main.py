...
@app.route("/create-post/<int:course_id>", methods=["GET", "POST"])
def create_post(course_id):
    if request.method == "POST":
        ...
        author = User.get_user_by_id(SESSION.current_user_id)

        # Create the post
        post = Post(
            id=None,  # Assuming the ID will be auto-generated in the DB
            created=datetime.datetime.now(),  # Set the created time to the current time
            title=title,
            content=content,
            author=author,  # Pass the User object here
            course_id=course_id,
            image_url=image_url
        )
        ...
        return redirect(url_for('course', course_id=course_id))  # Redirect to the course page

    return render_template('create_post.html', course=Course.query.get(course_id))  # Existing template for creating posts
